...
def format_content(text):
    ...
    # 2. 텍스트와 표가 붙어있는 경우 줄바꿈 추가 (예: ...많겠음.|구분|)
    # '파이프나 줄바꿈이 아닌 문자' 뒤에 '파이프'가 오면 줄바꿈 삽입
    text = re.sub(r'([^|\n])\s*\|', r'\1\n|', text)
    
    # 3. 표의 헤더와 구분선이 붙어있는 경우 줄바꿈 (예: |강수량||---|)
    # 파이프 뒤에 파이프+하이픈(-)이 오면 줄바꿈
    text = re.sub(r'\|\s*(\|\s*-)', r'|\n\1', text)
    
    # 4. 일반적인 행 분리 (파이프 뒤에 파이프가 오는데, 그 뒤에 숫자가 오거나 공백이면?)
    # 안전하게: 파이프 간격만 조금 벌려줌 (기존 로직 유지하되 순서 변경)
    text = text.replace('|', ' | ') 
